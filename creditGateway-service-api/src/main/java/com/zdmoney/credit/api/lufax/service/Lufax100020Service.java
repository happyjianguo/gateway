 ...
 */
@Service
public class Lufax100020Service extends LufaxBusinessService {
	...
	private static final String INTERFACE_ID = "810010";
    @FuncIdAnnotate(value = "lufax100020", desc = "同步追偿还款计划", voCls = Lufax100020Vo.class)
    public FuncResult execute(Lufax100020Vo vo) {
        try {
        	String jsonStr = JSON.toJSONString(vo,new CustomBigDecimalSerializer());
        	LOGGER.info("Lufax100020Service-同步追偿还款计划接口-业务数据" + jsonStr);
            ...
            if(HttpStatus.SC_OK == Integer.parseInt(json.getString("HttpStatus"))){
            	LOGGER.info("证大同步追偿还款计划给陆金所：文件名【"+fileName+"】上传到iobs服务器，" + json.getString("httpContent"));
