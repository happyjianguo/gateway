 ...
 */
@Service
public class Lufax100012Service extends LufaxBusinessService {
    ...
    @FuncIdAnnotate(value = "lufax100012", desc = "证大同步还款记录给陆金所", voCls = Lufax100012Vo.class)
    public FuncResult execute(Lufax100012Vo vo) {
        try {
        	String jsonStr = JSON.toJSONString(vo,new CustomBigDecimalSerializer());
        	LOGGER.info("Lufax100012Service-证大同步还款记录给陆金所-业务数据" + jsonStr);
            ...
            if(HttpStatus.SC_OK == Integer.parseInt(json.getString("HttpStatus"))){
            	LOGGER.info("证大同步还款记录给陆金所：文件名【"+fileName+"】上传到iobs服务器，" + json.getString("httpContent"));
