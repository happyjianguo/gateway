 ...
 */
@Service
public class Lufax100010Service extends LufaxBusinessService {
    
    private static final Log LOGGER = LogFactory.getLog(Lufax100010Service.class);
    ...
    @FuncIdAnnotate(value = "lufax100010", desc = "证大同步还款计划给陆金所", voCls = Lufax100010Vo.class)
    public FuncResult execute(Lufax100010Vo vo) {
        try {
        	String jsonStr = JSON.toJSONString(vo,new CustomBigDecimalSerializer());
        	LOGGER.info("Lufax100010Service-证大同步还款计划给陆金：" + jsonStr);
            ...
            if(HttpStatus.SC_OK == Integer.parseInt(json.getString("HttpStatus"))){
            	LOGGER.info("证大同步还款计划给陆金所：文件名【"+fileName+"】上传到iobs服务器，" + json.getString("httpContent"));
