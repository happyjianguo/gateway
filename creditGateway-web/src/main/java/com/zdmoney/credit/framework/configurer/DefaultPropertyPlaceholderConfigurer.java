		...
		try {
			...
			case PRODUCTION:
				...
				if (Strings.isNotEmpty(props.getProperty(JDBC_USER_NAME_KEY))) {
					...
					jdbcUserName = new String(ThreeDes.decryptMode(ThreeDes.keyBytes,
							ThreeDes.hex2byte(jdbcUserName.getBytes())));
				...
				if (Strings.isNotEmpty(props.getProperty(JDBC_PASS_WORD_KEY))) {
					...
					jdbcPassWord = new String(ThreeDes.decryptMode(ThreeDes.keyBytes,
							ThreeDes.hex2byte(jdbcPassWord.getBytes())));
